...
# 7. SHAP Analysis (Based on Best AUC Model)
best_overall = results_df.iloc[results_df['AUC'].idxmax()]
best_features = [col for g in best_overall['Group'].split('+') for col in groups[g]]
X_best = X[best_features]

explainer_model = RandomForestClassifier(n_jobs=-1, random_state=42).fit(X_best, y)
...
shap.summary_plot(s_values, X_best, show=False)
plt.title(f"SHAP Analysis: {best_overall['Group']}")
plt.savefig(f"{folder_name}/shap_analysis.png", dpi=300, bbox_inches='tight')
